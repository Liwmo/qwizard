...
describe('Question Directive', function() {
    ...
    it('scope.type is templated correctly when tf', inject(function($controller) {
        expect(element.children()[5].children.length).toBe(2);
        expect(element.children()[5].children[0].innerHTML).toBe("True");
        expect(element.children()[5].children[1].innerHTML).toBe("False");
    }));

    it('scope.selected should update when true is clicked', inject(function($controller){
        expect($scope.selected).toEqual([]);
        var el = angular.element(element.children()[5].children[0]);
